...
export const OrderSuccess = ({data}) => {
    return (
        <section className="text-xl text-center max-w-4xl mx-auto my-10 py-5 dark:text-slate-100 border dark:border-slate-700 rounded">
            <div className="my-5">
                <p className="bi bi-bag-check text-green-600 text-7xl mb-5"></p>
                <p>Excellent Choice {data.user.name}!</p>
                <p>Your Order ID: {data.user.id}</p>
            </div>
            <div className="my-5">
                <p>Order Confimed</p>
                <p>Your eBook has been dispatched to eMail - {data.user.email}</p>
                <p className="my-5">Payment ID: {data.user.id + 1000}</p>
            </div>
